...
export function deleteSkipReasonById(db: Database.Database, id: number): void {
    db.prepare(`DELETE FROM skip_reasons WHERE id = ?`).run(id);
}

export function deleteSkipReasonsByRoleId(db: Database.Database, roleId: number): void {
    db.prepare(`DELETE FROM skip_reasons WHERE role_id = ?`).run(roleId);
}