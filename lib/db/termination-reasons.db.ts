...
export function getTerminationReasonsByRoleId(
  db: Database.Database,
  roleId: number
...
export function getTerminationReasonById(
  db: Database.Database,
  id: number
): TerminationReasonRow | undefined {
  return db
    .prepare(
      `
    SELECT id, role_id, reason, note
...
export function deleteTerminationReasonsByRoleId(
  db: Database.Database,
  roleId: number
): Database.RunResult {
