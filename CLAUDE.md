...
- **Unit tests (`tests/unit/`, `client/tests/unit/`).** Goal: verify one function or module's behavior in total isolation — no I/O, no browser, no other layer involved. Scope: two distinct things — (a) business logic with real branches worth enumerating, and (b) a framework-integration contract that's cheap to verify here and easy to silently break elsewhere. Trigger: ask whether the thing under test has a non-obvious contract that's genuinely isolable from rendering/HTTP/DB. If the "test" would just restate control flow already fully visible in the source — call A, then B, then C, in the order the code already visibly does — it isn't testing a contract, it's re-describing the implementation; skip it.
