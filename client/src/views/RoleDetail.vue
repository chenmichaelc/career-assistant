<template>
  ...
  <div v-else-if="role">
    ...
    <div class="flex items-center justify-between mb-6">
      <router-link to="/" class="font-mono text-dim text-sm hover:text-text transition-colors"
        >← roles</router-link
      >
      <div class="flex gap-3">
        <button
          @click="showExport = true"
          class="border border-border text-dim font-mono text-sm px-3 py-1.5 rounded hover:text-text transition-colors"
        >
          export
        </button>
        <button
          @click="showDelete = true"
          class="border border-danger text-danger font-mono text-sm px-3 py-1.5 rounded hover:bg-danger/10 transition-colors"
        >
          delete
        </button>
      </div>
    ...
    <div class="bg-panel border border-border rounded p-4 mb-6">
      ...
      <div v-if="statusSuccess" class="font-mono text-success text-xs mt-2">
        {{ statusSuccess }}
      </div>
    </div>
    ...
    >
      ...
      <div v-if="addSkipReasonError" class="font-mono text-danger text-xs mt-2">
        {{ addSkipReasonError }}
      </div>
    </div>
    ...
    >
      ...
      <div v-if="addTerminationReasonError" class="font-mono text-danger text-xs mt-2">
        {{ addTerminationReasonError }}
      </div>
    </div>
    ...
    >
      <div class="bg-panel border border-border rounded p-6 w-full max-w-md mx-4">
        ...
        <div
          class="bg-surface border border-border text-dim font-mono text-sm px-3 py-2 rounded mb-4 opacity-70"
        >
          {{ pendingStatus }}
        </div>
    ...
    >
      <div class="bg-panel border border-border rounded p-6 w-full max-w-2xl mx-4">
        ...
        <div class="flex gap-3 mb-4">
          <button
            @click="exportFormat = 'simple'"
            :class="
              exportFormat === 'simple' ? 'bg-accent text-surface' : 'border border-border text-dim'
            "
            class="font-mono text-sm px-3 py-1.5 rounded transition-colors"
          >
            simple
          </button>
          <button
            @click="exportFormat = 'rich'"
            :class="
              exportFormat === 'rich' ? 'bg-accent text-surface' : 'border border-border text-dim'
            "
            class="font-mono text-sm px-3 py-1.5 rounded transition-colors"
          >
            rich
        ...
        <pre
          class="bg-surface border border-border text-text font-mono text-xs p-4 rounded overflow-auto max-h-96 whitespace-pre-wrap"
          >{{ exportContent }}</pre
        >
    ...
    >
      <div class="bg-panel border border-border rounded p-6 w-full max-w-lg mx-4">
        ...
        <div class="font-mono text-xs text-dim mb-4">
          ...
          {{ role.termination_reasons?.length ?? 0 }} termination reasons, 1 job description
        </div>
        <div class="flex gap-3">
          <button
            @click="confirmDelete(false)"
            class="border border-danger text-danger font-mono text-sm px-4 py-2 rounded hover:bg-danger/10 transition-colors"
          >
            delete if clean
          </button>
          <button
            @click="confirmDelete(true)"
            class="bg-danger text-surface font-mono text-sm px-4 py-2 rounded hover:opacity-90 transition-opacity"
          >
            force delete all
          </button>
          <button
...
import { VALID_STATUSES, VALID_SKIP_REASONS, VALID_TERMINATION_REASONS } from '@/constants';
...
async function loadExport() {
  try {
    const data = await apiFetch<{ content: string }>(
      `/api/roles/${route.params.id}/export?format=${exportFormat.value}`
    );
